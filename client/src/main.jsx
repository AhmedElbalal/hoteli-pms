...
function Login({ onLogin }) {
  ...
  return <div className="login"><form className="card" onSubmit={submit}>
    <h1>HOTELI</h1><p className="muted"> Hotel Cloud PMS </p>
    ...
    <p className="muted"> admin@hoteli.com / demo123</p>
  </form></div>;
}
...
function Reservations({ t, reload, data }) {
  ...
  async function save() {
    setMsg('');
    try { await api('/reservations', { method: 'POST', body: JSON.stringify({ ...f, adults: Number(f.adults), rate: Number(f.rate), parking: Boolean(f.parking) }) }); setMsg(t.saved); await reload(); await loadAvailability(); }
  ...
  return <div className="stack">
    <div className="card"><h2>{t.newReservation}</h2>{msg && <p className="pill amber">{msg}</p>}
      <div className="form">
        <input className="input" placeholder={t.guest} value={f.guestName} onChange={e => setF({ ...f, guestName: e.target.value })} />
      ...
      </div><button className="btn" onClick={save}>{t.save}</button>
    </div>
...
function Folios({ t }) {
  ...
  return <div className="grid2"><div className="card"><h2>{t.folios}</h2><table className="table"><tbody>{rows.map(f => <tr key={f.id} onClick={() => open(f.id)} className="clickable"><td><b>{f.id}</b><br />{f.guestName}</td><td>{money(f.balance)}</td></tr>)}</tbody></table></div>{selected && <div className="card"><h2>{t.folioContent}: {selected.id}</h2><p><b>{selected.guestName}</b> · <span className="pill">{selected.status}</span></p><table className="table"><thead><tr><th>{t.date}</th><th>{t.code}</th><th>{t.description}</th><th>{t.amount}</th></tr></thead><tbody>{selected.items.map(i => <tr key={i.id}><td>{i.date}</td><td>{i.code}</td><td>{i.description}</td><td>{money(i.amount)}</td></tr>)}</tbody></table><h2>{t.balance}: {money(selected.balance)}</h2><p className="muted">Room folios include room charge, TPS, TVQ, taxe d’hébergement. Parking includes TPS and TVQ.</p><div className="form"><input className="input" type="number" step="0.01" value={payment} onChange={e => setPayment(e.target.value)} placeholder={t.payment} /><button className="btn" onClick={pay}>{t.postPayment}</button></div></div>}</div>;
}
