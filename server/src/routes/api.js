...
function folioBalance(folio) {
  return (folio.items || []).reduce((sum, item) => sum + Number(item.amount || 0), 0);
}
function addFolioItem(folio, { type = 'CHARGE', code, description, amount, date }) {
  folio.items.push({ id: `FI-${nanoid(8)}`, type, code, description, amount: Number(amount.toFixed ? amount.toFixed(2) : amount), date });
}
...
api.get('/availability', async (req, res) => {
  ...
  const rooms = db.rooms.map(room => ({
    ...room,
    available: roomIsAvailable(db, room.number, checkIn, checkOut),
    suggestedRate: room.rateBase,
    taxesPreview: {
      roomCharge: room.rateBase,
      tps: Number((room.rateBase * QC_TAX.tps).toFixed(2)),
      tvq: Number((room.rateBase * QC_TAX.tvq).toFixed(2)),
      lodgingTax: Number((room.rateBase * QC_TAX.lodging).toFixed(2)),
      total: Number((room.rateBase * (1 + QC_TAX.tps + QC_TAX.tvq + QC_TAX.lodging)).toFixed(2))
    }
  }));
